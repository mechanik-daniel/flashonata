...
var flashtevalWithCallback = function(expr, data, bindings) {
...
describe('Invoke JSONata with callback', function() {
    describe('Make HTTP request', function() {
        it('should return promise to results', function() {
            var expr = flashteval('$httpget("https://api.npmjs.org/downloads/range/2016-09-01:2017-03-31/flashteval").downloads{ $substring(day, 0, 7): $sum(downloads) }');
            expr.assign('httpget', httpget);
            return expect(flashtevalWithCallback(expr)).to.eventually.deep.equal({
                '2016-09': 205,
                '2016-10': 1266,
                '2016-11': 2783,
                '2016-12': 2158,
...
describe('Invoke JSONata with callback - errors', function() {
    describe('type error', function() {
        it('should throw', function() {
            var expr = flashteval('5 + $httpget("htttttps://api.npmjs.org/downloads/range/2016-09-01:2017-03-31/flashteval")');
            expr.assign('httpget', httpget);
            return expect(flashtevalWithCallback(expr)).to.be.rejected;
        });
    ...
    describe('Make HTTP request with dodgy url', function() {
        it('should throw', function() {
            var expr = flashteval('$httpget("htttttps://api.npmjs.org/downloads/range/2016-09-01:2017-03-31/flashteval").downloads{ $substring(day, 0, 7): $sum(downloads) }');
            expr.assign('httpget', httpget);
            return expect(flashtevalWithCallback(expr)).to.be.rejected;
        });
    });
...
describe('Invoke JSONata with callback - return values', function() {
    it('should handle an undefined value', function() {
        ...
        var expr = flashteval('value');
        return expect(flashtevalWithCallback(expr, data)).to.eventually.equal(undefined);
    });
    it('should handle a null value', function() {
        var data = { value: null };
        var expr = flashteval('value');
        return expect(flashtevalWithCallback(expr, data)).to.eventually.equal(null);
    });
    it('should handle a value', function() {
        var data = { value: 'hello' };
        var expr = flashteval('value');
        return expect(flashtevalWithCallback(expr, data)).to.eventually.equal('hello');
    });
...
describe('Evaluate concurrent expressions with callbacks', function() {
    it('should process two expressions concurrently', function(done) {
        const expr = flashteval("{'1':'goat','2': 'cheese'} ~> $lookup($string(payload))");
...
describe('Handle chained functions that end in promises', function() {
    ...
    it('basic async function', function() {
        var data = {};
        var expr = flashteval('$counter(5).value');
    ...
    it('basic async function, but invokes another function', function() {
        var data = {};
        var expr = flashteval('$counter(0).inc().value');
    ...
    it('basic async function, but invokes another function several times', function() {
        var data = {};
        var expr = flashteval('$counter(0).inc().inc().inc().inc().value');
    ...
    it('basic async function and part of a numeric expression', function() {
        var data = {};
        var expr = flashteval('$counter(3).value + 5');
    ...
    it('basic async function, but invokes another function several times and part of a numeric expression', function() {
        var data = {};
        var expr = flashteval('$counter(0).inc().inc().inc().inc().value + 3');
    ...
    it('basic async function, but invokes another function - nested', function() {
        var data = {};
        var expr = flashteval('$counter($counter(3).inc().inc().value).inc().value');
    ...
    it('basic async function, then invokes a built-in function', function() {
        var data = {};
        var expr = flashteval('$counter(3).inc().value.$string()');
        return expect(expr.evaluate(data, bindings)).to.eventually.equal('4');
    });

    it('basic async function, but invokes a non-existent function', function() {
        var data = {};
        var expr = flashteval('$counter(2).inc().foo().value');
